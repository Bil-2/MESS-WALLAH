 ...
 * Seeds production MongoDB with complete India room data
 * 
 * Usage:
 *   1. Set MONGODB_URI_PRODUCTION in .env.production
...
require('dotenv').config(); // Fallback to regular .env

// Use production MongoDB URI (priority order)
const MONGODB_URI = process.env.MONGODB_URI_PRODUCTION || process.env.MONGODB_URI;

console.log('\n' + '='.repeat(70));
console.log('  PRODUCTION DATABASE SEEDING - MESS WALLAH');
console.log('='.repeat(70));
console.log(`📍 Target: ${MONGODB_URI.replace(/:([^:@]+)@/, ':****@')}`);
console.log('📊 Will add: 4,438 rooms across 28 states, 739 districts');
console.log('='.repeat(70) + '\n');
...
function askConfirmation() {
  return new Promise((resolve) => {
    rl.question('⚠️  Proceed with PRODUCTION database seeding? Type YES to confirm: ', (answer) => {
      rl.close();
      resolve(answer === 'YES');
    });
  });
...
(async () => {
  ...
  if (!confirmed) {
    console.log('\n❌ Seeding cancelled. Type YES (all caps) to proceed.\n');
    process.exit(0);
  }

  console.log('✅ Confirmation received. Starting seeding...\n');
  await seedDatabase();
})();
