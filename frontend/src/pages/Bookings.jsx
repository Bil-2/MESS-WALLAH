...
const Bookings = () => {
  ...
  const handleDownloadReceipt = async (bookingId) => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `MESS-WALLAH-Receipt-${bookingId}.pdf`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
      toast.success('Receipt downloaded!', { id: 'pdf-dl' });
    } catch (error) {
      console.error('Error downloading receipt:', error);
      toast.error('Failed to download receipt. Try again later.', { id: 'pdf-dl' });
    }
  };
